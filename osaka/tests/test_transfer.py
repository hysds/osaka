...
class TransferTest(unittest.TestCase):
    ...
    def test_InOuts(self,callback=None):
        ...
        for inout in self.inouts:
            osaka.utils.LOGGER.info("Running In-Out Test for {0}".format(inout))
            ...
            for remote in objs:
                ...
                self.assertTrue(self.checkObject(loc),"Downloaded product inconsistent with original product: {0}".format(loc))
    ...
    def test_InOutExists(self):
        ...
        def reupload(download,remote):
            ...
            if not os.path.isdir(download):
                self.checkObject(os.path.join(scratch,os.path.basename(remote)))
            else:
                self.checkObject(os.path.join(scratch,os.path.basename(remote),os.path.basename(remote)))
    ...
    def test_Criscross(self,extras=[]):
        ...
        for inout in self.inouts:
            try:
                osaka.utils.LOGGER.info("Running Criscross Test for {0}".format(inout))
                ...
                for remote in objs:
                    ...
                    loc = noquery.sub("", os.path.join(self.scratch,os.path.basename(remote)))
                    self.assertTrue(self.checkObject(loc),"Downloaded product inconsistent with original product: {0}".format(loc))
                    osaka.main.rmall(loc)
                #Cleanup
    ...
    def uploadInputObjects(self,uriBase):
        ...
        for obj in self.objects:
            self.assertFalse(osaka.main.exists(os.path.join(dest,os.path.basename(obj))), "Destination {0} already exists, test cannot safely continue.".format(dest))
