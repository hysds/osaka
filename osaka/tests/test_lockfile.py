...
class LockfileTest(unittest.TestCase):
    ...
    def test_lock_source(self):
        ...
        for obj in self.ins:
            ...
            lock = osaka.lock.Lock(os.path.join(self.scratch,os.path.basename(obj)))
            self.assertTrue(os.path.exists(self.lockfile.format(base)), "Lockfile {0} does not exist".format(self.lockfile.format(base)))
    ...
    def test_collaborate_and_error(self):
        ...
        for obj in self.ins:
            ...
            try:
                ...
                self.assertFalse(True, "No error thrown when cooperating on an errored download")
            except Exception as exc:
                exp_error = lock.getLockMetadata("error")
                exp_error = "dsakhldsalkjsdaasd" if exp_error is None else exp_error
                self.assertTrue(exp_error in str(exc), "Error {0} not found in cooperation error {1}".format(exp_error, str(exc)))
            ...
            with self.assertRaises(osaka.utils.CooperationRefusedException):
                osaka.main.get(obj, self.scratch, ncoop=True, params={"timeout":2})
