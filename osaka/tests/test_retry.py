...
class RetryTest(unittest.TestCase):
    ...
    def test_retry(self):
        ...
        for obj in self.ins:
            ...
            time1 = time.time()
            try:
                osaka.main.get(obj,self.scratch, {"timeout":0.00000001},retries=100)
            except:
                pass
            time1 = time.time() - time1
            #Make sure it is at least 10 times longer
            osaka.utils.LOGGER.info("Time0: {0} Time1: {1}:".format(time0,time1))
            self.assertTrue(time1/time0 >= 9, "Retry didn't occur enough {0} vs {1}".format(time1/time0, 10))
