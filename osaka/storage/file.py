    ...
    def close(self):
        ...
        for fh in self.files:
            try:
                ...
                osaka.utils.LOGGER.debug("Failed to close file-handle for: {0}".format(fh.name))
    def rm(self,uri):
        '''
...
class FileHandlerConversion():    
    ...
    def __init__(self,stream):
        ...
        if self.filename is None or not os.path.exists(self.filename):
            self.handler = File()
            self.filename = "/tmp/osaka-temporary-"+datetime.datetime.utcnow().strftime("%Y%m%d%H%M%S.%f")
