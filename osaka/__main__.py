...
def get_main_parser():
    ...
    parser.add_argument("-v", "--verbose", help="Enables verbose output",action="store_true",default=False)
    parser.add_argument("-d", "--debug", help="Enables debug output",action="store_true",default=False)
    ...
    parser_ex = subparsers.add_parser("gojira", description = "RRRRRRAAAAAAAWWW...", help="RRRRRRAAAAAAAWWW...")
    return parser
def add_get_parser(subparsers):
    ...
    parser_get.add_argument("-f", "--force", help="Forces a retrieval of the source Osaka-URL",action="store_true",default=False)
    parser_get.add_argument("-x", "--no-coop", help="Refuses to cooperate with other osaka processes",action="store_true",default=False, dest="ncoop")
    parser_get.add_argument("-n", "--no-clobber", help="Refuses to clobber existing destinations",action="store_true",default=False, dest="noclobber")
...
def add_put_parser(subparsers):
    ...
    parser_put.add_argument("-x", "--no-coop", help="Refuses to cooperate with other osaka processes",action="store_true",default=False, dest="ncoop")
    parser_put.add_argument("-n", "--no-clobber", help="Refuses to clobber existing destinations",action="store_true",default=False, dest="noclobber")
...
def add_rm_parser(subparsers):
    ...
    parser_rm.add_argument("source", type=functools.partial(argparse_exception_wrapper,osaka.base.StorageBase.getStorageBackend),
                                  help="The source Osaka-URL to remove")
    parser_rm.add_argument("-f","--force", dest="unlock", help="Force unlock before removal",action='store_true',default=False)
...
def add_size_parser(subparsers):
    ...
    parser_size.add_argument("-f","--force", dest="force", help="Force sizing for locked urls",action='store_true',default=False)
...
def add_list_parser(subparsers):
    ...
    parser_ls.add_argument("-f","--force", dest="force", help="Force sizing for locked urls",action='store_true',default=False)
...
def main(args=None):
    ...
    try:
        ...
        print("[ERROR] An exception of type {0} occured with message '{1}'".format(type(e).__name__,e))
        if parsed.debug:
            traceback.print_exc()
        return -1
    return 0   
