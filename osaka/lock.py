...
class Lock(object):
    ...
    def getLockMetadata(self, field):
        ...
        '''
        osaka.utils.LOGGER.debug("Looking for {0} in lock-uri {1}".format(field, self.luri))
        with PermTemp(self.luri, self.handle, self.params) as handle: 
            try:
...
#            return
    ...
    def isLocked(self):
        ...
        '''
        osaka.utils.LOGGER.info("Checking lock status of {0} in {1}".format(self.ouri, self.luri))
        if self.locked:
            return self.locked
        try:
...
class PermTemp(object):
    ...
    def __init__(self, luri, handle=None, params={}):
        ...
        if handle is None:
            self.close = True
            osaka.utils.LOGGER.debug("Opening handler for lock-uri {0}".format(self.luri))
